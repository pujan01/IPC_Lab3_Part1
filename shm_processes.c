...
int  main(int  argc, char *argv[])
{
  ...
  int fd[2];  
  // declaring an array to put integers
  int buffer[10];

  // using pipe
  pipe(fd);

  //using exactly 10 arguments
  if (argc != 11) {
...
//method signature has been changed to include two parameters
void  ClientProcess(int  SharedMem[], int fd[]) 
{
  ...
  printf("Writing to the pipe is done\n");
